...
Trả về danh sách bài viết mà user hiện tại đã bookmark.  
Sắp xếp theo thời gian bookmark mới nhất lên đầu.  
...
| BR2 | Chỉ trả về bài của `request.user` đã bookmark                               |
| BR3 | Chỉ trả về bài có `is_deleted=False`                                        |
| BR4 | Sắp xếp theo thời gian bookmark mới nhất (`bookmarks__created_at` DESC)     |
| BR5 | Phân trang `limit` (default `10`, max `100`) / `offset` (default `0`)      |
| BR6 | `is_bookmarked` luôn là `true` trong response này                            |
...
| ------------ | --------- | ----------------------------------------------- |
| `bookmarks`  | JOIN      | Filter theo `user_id = request.user.id`         |
...
| File                                                | Mô tả                          |
| --------------------------------------------------- | ------------------------------ |
| [UC18/19 — Bookmark/Unbookmark](uc18-19-bookmark.md) | Toggle bookmark               |
